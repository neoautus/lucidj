...
package xyz.kuori.common_parts;
...
{
    @Property public String title = "Common UI Elements";
    @Property public int weight = 100;
    @Property public Resource icon = FontAwesome.DASHBOARD;

    @Property(name="View-Caption")
    private String caption = "Common UI Elements";
    ...
    private ObjectRenderer test_renderer;
    ...
    private void buildView()
    {
        ...
        // ---GridStack---
        //GridStackLayout row = new GridStackLayout(8); // => For GridStack, uncomment this
        // ---/GridStack

        row.addStyleName("my-gridstacklayout");
        row.setWidth("100%");
        addComponent(row);

        row.addComponent (osgiHacks ());
        row.addComponent(loadingIndicators());
        row.addComponent(notifications());
        row.addComponent(windows());
        row.addComponent(tooltips());

        row.addComponent(getMenuButton("Teste!"));
    }

    Panel osgiHacks ()
    {
        Panel p = new Panel("OSGi Hacks");
        final VerticalLayout content = new VerticalLayout();
        p.setContent(content);
        content.setSpacing(true);
        content.setMargin(true);

        Bundle bnd = ctx.getBundle ();
        BundleWiring bw = bnd.adapt(BundleWiring.class);
        ClassLoader cld = bw.getClassLoader();


        content.addComponent (new Label("Component classloader: " + this.getClass ().getClassLoader ().toString ()));
        content.addComponent (new Label("Bundle classloader: " + cld.toString ()));
        content.addComponent (new Label ("Context classloader: " + Thread.currentThread ().getContextClassLoader ().toString ()));
        content.addComponent (new Label ("Panel classloader: " + p.getClass ().getClassLoader ().toString ()));
        content.addComponent (new Label ("Quark classloader: " + Quark.class.getClassLoader ().toString ()));
        content.addComponent (new Label ("SAPI classloader: " + sapi.getClass ().getClassLoader ().toString ()));

        test = new TestComponent ();
        test.setValue ("HELLO WORLD!");

        test_renderer = new ObjectRenderer (ctx);
        content.addComponent (test_renderer.link (test));

        content.addComponent (new ObjectRenderer (ctx).link (test));

        TestComponent another = new TestComponent ();
        another.setValue ("The quick brown fox jumped over the lazy dogs!");
        content.addComponent (new ObjectRenderer (ctx).link (another));
        ...
        wait.addClickListener(new ClickListener() {
            ...
            public void buttonClick(final ClickEvent event) {
                try {
                    for (int i = 0; i < 15; i++)
                    {
                        test.setValue ("Counting " + i);
                        test_renderer.updateComponent ();
                        Thread.sleep(1000);
                    }
                } catch (InterruptedException e) {
                }
...
//            TextArea description = new TextArea("Description");
            AceEditor description = new AceEditor ();
            //description.
                ...
                description.addValueChangeListener(new ValueChangeListener() {
                    ...
                    public void valueChange(final ValueChangeEvent event) {
                        ...
                        if (description.getValue() == null
                            ...
                            description.setHeight(editorLineHeight + 2, Unit.PIXELS);
                        } else {
                            notification.setDescription(description.getValue());
                            int lines = text.length() - text.replace("\n", "").length();
                            description.setHeight(editorLineHeight * (lines + 1) + 2, Unit.PIXELS);
                        }
                    }
                });
                description.setHeightUndefined();
                description.setValue("The quick brown fox jumped over the lazy dogs.");
                description.setHeight(editorLineHeight + 2, Unit.PIXELS);
*/

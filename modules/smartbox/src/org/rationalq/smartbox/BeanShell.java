...
public class BeanShell
{
    ...
    class PipeListener extends Thread
    {
        ...
        public PipeListener (PipedOutputStream source, PrintStreamListener listener)
        {
            ...
            log.debug ("PipeListener {}: source={}, listener={}, sink={}", this, source, listener, sink);
        }

        @Override
        public void run ()
        {
            log.debug ("PipeListener {}: START", this);
            try
            {
                int len;
                ...
                while ((len = sink.read (buffer, 0, buffer.length)) != -1)
                {
                    ...
                    log.debug ("PipeListener {}: OUTPUT [{}]", sink, output.replace ("\n", "\\n"));
            ...
            log.debug ("PipeListener {}: FINISH", this);
        }
    }
